...
//res = P + Q
	...
	// Debug: Check if nacl parameter is valid
	if (typeof nacl === 'undefined') {
		throw new Error('nacl parameter is undefined inside IIFE');
	}
	
	// Debug: Check what type of object nacl is
	if (typeof nacl !== 'object') {
		throw new Error('nacl parameter is not an object: ' + typeof nacl);
	}
	
	// Debug: Check if functions exist before assignment
	if (typeof ge_scalarmult_base !== 'function') {
		throw new Error('ge_scalarmult_base function not defined inside IIFE');
	}
	
	if (typeof ge_scalarmult !== 'function') {
		throw new Error('ge_scalarmult function not defined inside IIFE');
	}
	
	// Debug: Check if nacl is writable
	try {
		nacl.test_property = 'test';
		if (nacl.test_property !== 'test') {
			throw new Error('nacl object is not writable');
		}
		delete nacl.test_property;
	} catch (e) {
		throw new Error('nacl object is not writable: ' + e.message);
	}
	...
	try {
		...
		// Debug: Check if nacl.ll was assigned immediately after
		if (!nacl.ll) {
			throw new Error('nacl.ll assignment failed - nacl.ll is still undefined');
		}
		
		// Debug: Check if ge_scalarmult_base was assigned
		if (!nacl.ll.ge_scalarmult_base) {
			throw new Error('nacl.ll.ge_scalarmult_base assignment failed');
		}
		
	} catch (e) {
		throw new Error('Error during nacl.ll assignment: ' + e.message);
	}
	...
	(function() {
		...
		if (crypto && crypto.getRandomValues) {
			...
			// Node.js or React Native with polyfill.
			try {
				crypto = require('crypto');
			} catch (e) {
				// Fallback for React Native - use global crypto if available
  ...
  // Always use global.nacl in React Native
  if (typeof global !== 'undefined') {
    if (!global.nacl) {
      global.nacl = {};
    }
...
if (typeof global !== 'undefined' && global.nacl) {
  if (!global.nacl.ll) {
    throw new Error('IIFE failed - nacl.ll is undefined');
  }
  if (!global.nacl.ll.ge_scalarmult_base) {
    throw new Error('IIFE failed - ge_scalarmult_base is undefined');
  }
}